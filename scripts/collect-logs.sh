...
case "$OSTYPE" in
  linux-gnu*) APP_LOG_DIR="$HOME/.local/share/com.projectsf.patient-manager/logs" ;;
  darwin*)    APP_LOG_DIR="$HOME/Library/Logs/PatientManager" ;;
...
if [[ "$DRY_RUN" == true ]]; then
  ...
  find "$APP_LOG_DIR" -type f -printf "  → %f\n" 2>/dev/null || true
else
