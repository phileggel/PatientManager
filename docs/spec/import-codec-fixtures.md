...
content visible to downstream code, with two explicit carve-outs — (a)
session-scoped values the parser regenerates on every parse (this rule),
and (b) transport-metadata fields that describe the source document's
physical layout (defined in IFC-026). For the Excel surface, carve-out (a)
covers every session-scoped UUID the parser regenerates: `temp_id` on each
`ExcelPatient` and `ExcelFund`, and `procedure_type_tmp_id` plus the
foreign-key references `patient_temp_id` and `fund_temp_id` on each
`ExcelProcedure` (per EXI R5: all of these are session-scoped, never
persisted; the FK references inherit the carve-out because their values
are derived from parent IDs the parser also regenerates). The round-trip
...
**IFC-026 — Transport metadata excluded from round-trip equality (backend)**:
Some parser-populated fields on the contract describe the source document's
physical layout (e.g. which row number a parsed entry came from within its
sheet) rather than the domain content of the row. The codec round-trip
equality (IFC-021) does NOT apply to these transport-metadata fields:
scenarios do not specify them, the generator assigns them based on its own
layout strategy when writing the file, and the round-trip assertion ignores
them. For the Excel surface, this carve-out covers the source row number on
each `ExcelProcedure`. (The sheet name is NOT transport metadata under this
rule — it is logical content already captured by `ExcelProcedure.sheet_month`
and IS part of round-trip equality per IFC-021.) Transport-metadata fields
remain deterministic per IFC-040; the carve-out is from round-trip equality
only. Field additions of this kind are driven by non-codec features
(execute-time robustness — see EXI-280 and EXI-290) and so fall outside
IFC-022's restriction on codec-driven parser-shape changes.
...
Additional scenarios may be added without changing existing ones. Scenario rows whose `procedure_date.month` does not equal their `sheet_month`'s nominal month exercise EXI-281's execute-time skip path — scenarios that intend this MUST declare the resulting execute-time skip separately (it surfaces in `ImportExecutionResult.skipped_procedures`, not in `parsing_issues.skipped_rows` which is parse-time only). The codec round-trip property (IFC-021) does NOT cover execute-time skips; only parse-time `parsing_issues` is in scope of the round-trip equality.
