...
**FPA-025 — Inverted period dates (backend)**: When a PDF line carries a period (`du <date> au <date>`) and the parsed start date is later than the end date, the parser silently swaps the two dates so the line is matched against the chronologically-forward range. Fund payments cover past procedures, so an inverted period is treated as a PDF typo, not a meaningful instruction. Format-agnostic: applies to both short (`DD/MM`) and full (`DD/MM/YYYY`) forms because the comparison runs on parsed `NaiveDate` values. A `tracing::warn!` entry is emitted at parse time so the inversion can be audited from backend logs.
