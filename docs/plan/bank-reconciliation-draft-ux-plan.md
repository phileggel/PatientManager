...
Source contract: `docs/contracts/bank-statement-auto-match-contract.md` (commands `compute_bank_reconciliation_draft`, `validate_bank_reconciliation`; 4 superseded).
...
- [ ] ✍️ Backend test stubs (`test-writer-backend` from contract: `compute_bank_reconciliation_draft`, `validate_bank_reconciliation` — red confirmed)
...
- [ ] ✍️ Frontend test stubs (`test-writer-frontend` — gateway unit, presenter unit, RTL list/modals/wizard; `modified_functions = [useReconciliationDraft.ts:applyCorrection, useReconciliationDraft.ts:revert]` for the `[unit-test-needed]` rules BAS-062/065 — red confirmed)
...
- **`draft.rs`** _(new)_ — the draft model + pure recompute engine:
  - Types: `ReconciliationCorrection` (`LinkFund { bank_label, assignment: FundAssignment }` / `AssignGroups { line_id, group_ids }` / `AcknowledgeRemainder { line_id }`), `FundAssignment` (`Fund { fund_id } | Rejected`), `ReconciliationDraft`, `DraftLine`, `DraftLineStatus` (`Matched/NeedsLink/NeedsGroup/Partial/Rejected/Unresolved`), `CandidateGroup`. All `Serialize/Deserialize/Type` (Specta).
  - `compute_draft(parse_result, bank_account_id, corrections, &repos) -> Result<ReconciliationDraft, _>`: load saved mappings + live unsettled groups → initial auto-match (reuse existing match logic, BAS-050–054) → replay corrections in order (link-fund cascade BAS-066, group consumption BAS-067, multi-group balance BAS-090/091, remainder BAS-092) → per-line status BAS-061 → candidate proposals BAS-068. Pure / read-only.
  - Rejection paths (BAS-064): `AssignmentOverflow` (BAS-094), `GroupNotEligible` (BAS-090), `GroupAlreadyConsumed` (BAS-067).
- **`orchestrator.rs`** _(modify)_ — **PR 1**: add `compute_draft` + `validate_reconciliation`; keep the auto-match / resolve / save / create-transfers internals (the draft engine reuses them as private helpers; the old public commands still call them). `validate_reconciliation` recomputes the draft then commits: upsert mappings (BAS-035, ADR-001), N transfers per assigned group (BAS-093), procedure status + group lock (BAS-071–073). Non-atomic (ADR-002/003 precedent). **PR 2**: remove the now-superseded public `match_against_unsettled_groups` / `create_transfers` / `resolve_fund_labels` / `save_label_mappings` (the reusable internals stay, used by the draft engine).
- **`api.rs`** _(modify)_ — **PR 1**: ADD the new Tauri commands `compute_bank_reconciliation_draft`, `validate_bank_reconciliation`. Keep ALL existing commands (incl. the 4 to-be-superseded) so the current FE still compiles. **PR 2**: delete `resolve_bank_fund_labels`, `save_bank_fund_label_mappings`, `match_bank_statement_lines`, `create_bank_transfers_from_statement` once the FE no longer calls them.
...
- **`gateway.ts`** _(modify)_ — add `computeBankReconciliationDraft`, `validateBankReconciliation` (positional args per bindings); **remove** `resolveBankFundLabels`, `saveBankFundLabelMappings`, `matchBankStatementLines`, `createBankTransfersFromStatement`. **PR 2 also lands the matching BE deletions** (`api.rs` / `specta_builder.rs` / `orchestrator.rs` public fns) so the old commands and their only callers vanish in the same merge — `tsc` stays green at every PR boundary.
- **`ui/useReconciliationDraft.ts`** _(new — replaces `useBankStatementModal.ts`)_ — owns `corrections: ReconciliationCorrection[]` + the recomputed `draft`; each apply/revert appends/drops a correction and re-calls `computeBankReconciliationDraft` (BAS-064/065); busy + typed-error state (F27); `validate()` calls `validateBankReconciliation`. Keeps the account-create gate (BAS-011–017) ahead of the list.
...
- **`shared/draftPresenter.ts`** _(new)_ — pure `DraftLineStatus`/error → i18n key + view-model (F27 layer 3). **`shared/draftCorrections.ts`** _(new)_ — pure helpers to build correction commands + balance math (`.ts`, not `.tsx`).
...
| BAS-061             | frontend + backend | `draft.rs` `DraftLineStatus` + `draftPresenter.ts`                         | 6-status set               |
| BAS-062             | frontend + backend | `ReconciliationList.tsx` dblclick + correction modals; `draft.rs` override | `[unit-test-needed]`       |
| BAS-063             | frontend + backend | `validate_reconciliation` skip-unresolved                                  |                            |
| BAS-064             | frontend + backend | `draft.rs::compute_draft` + `useReconciliationDraft.ts`                    | pure recompute; busy/error |
| BAS-065             | frontend + backend | `useReconciliationDraft.ts` revert + recompute                             | `[unit-test-needed]`       |
| BAS-066             | frontend + backend | `draft.rs` link cascade + `LinkFundModal.tsx`                              |                            |
| BAS-067             | backend            | `draft.rs` group consumption                                               |                            |
| BAS-068             | frontend           | `AssignGroupsModal.tsx` candidates + broaden                               |                            |
| BAS-069             | frontend           | `ReconciliationList.tsx` summary + filter                                  |                            |
| BAS-090             | frontend + backend | `draft.rs` multi-group eligibility + `AssignGroupsModal.tsx`               |                            |
| BAS-091             | backend            | `draft.rs` balance                                                         |                            |
| BAS-092             | frontend + backend | `draft.rs` remainder + `RemainderModal.tsx`                                | informational only         |
| BAS-093             | backend            | `validate_reconciliation` N transfers                                      |                            |
| BAS-094             | frontend + backend | `draft.rs` overflow + `AssignGroupsModal.tsx` guard                        |                            |
