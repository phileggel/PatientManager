//! Fund-PDF surface of the dev fixture generator (IFC-023, IFC-031, IFC-060).
//!
//! Owns the scenario builders (hardcoded `PdfParseResult` values) and the
//! PDF writer that turns each scenario into a `.pdf` file plus a sibling
//! `.expected.json` snapshot. Sibling to `fixtures_excel` per IFC-023 — the
//! two surfaces share NO traits, helpers, or constants.
...
type ScenarioBuilder = fn() -> PdfParseResult;
type ScenarioEntry = (&'static str, ScenarioBuilder);

/// Regenerate one or all scenarios for the fund-PDF surface.
///
...
pub fn regenerate(out_root: &Path, scenario: Option<&str>) -> Result<()> {
    ...
    let registry: &[ScenarioEntry] = &[
        (
            "happy_path_multi_fund",
            scenarios::happy_path_multi_fund as ScenarioBuilder,
        ),
        (
            "unparsed_line_present",
            scenarios::unparsed_line_present as ScenarioBuilder,
        ),
    ];
    ...
    if to_run.is_empty() {
        anyhow::bail!(
            "unknown scenario for surface 'fund-pdf': {}\nKnown: {}",
