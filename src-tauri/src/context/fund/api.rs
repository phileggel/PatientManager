...
pub async fn update_fund_payment_group_with_procedures(
    ...
    // Preserve existing total_amount (not editable from this endpoint)
    let existing = fund_payment_service
        .read_group(&group_id)
        .await
        .map_err(|e| format!("{:#}", e))?
        .ok_or_else(|| format!("Payment group not found: {}", group_id))?;

    fund_payment_service
        .update_group(group_id, payment_date, procedure_ids, existing.total_amount)
        .await
        .inspect(|group| {
