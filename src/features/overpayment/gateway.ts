...
export async function createOverpayment(
...
): Promise<ServiceResult<null, OverpaymentError>> {
  ...
  logger.error("Failed to create overpayment refund", { code: result.error.code });
...
export async function cancelOverpayment(
  ...
  logger.error("Failed to cancel overpayment refund", { code: result.error.code });
  return { success: false, error: result.error };
}

export async function getProcedureRefundBySource(
  sourceProcedureId: string,
): Promise<ServiceResult<ProcedureRefundInfo | null, OverpaymentError>> {
  ...
  logger.error("Failed to fetch procedure refund", { code: result.error.code });
  return { success: false, error: result.error };
}
...
export async function getProcedureRefundByRefundProcedure(
  ...
  logger.error("Failed to fetch procedure refund by refund procedure", { code: result.error.code });
  return { success: false, error: result.error };
}
